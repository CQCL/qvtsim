...
from qvtsim.estimation_class import QVEstimate
from qvtsim.utils import convert, estimate_errors
...
class ScalableEstimate(QVEstimate):
    ...
    def single(self,
        ...
        """
        tq_dep = (
            convert(1 - error_dict['tq_dep'], 4, 'avg', 'dep')
            * convert(1 - error_dict['tq_coh'], 4, 'avg', 'dep')
        )          
        tq_other = (
            convert(1 - error_dict['tq_dph'], 4, 'avg', 'dep')
            * convert(
                convert(1 - error_dict['tq_cross'], 2, 'avg', 'proc'), 
                4, 'proc', 'dep'
            )
        )
        sq_dep = convert(
            convert(
                convert(1 - error_dict['sq_dep'], 2, 'avg', 'dep')
                * convert(1 - error_dict['sq_coh'], 2, 'avg', 'dep')
                * convert(1 - error_dict['sq_dph'], 2, 'avg', 'dep'),
                2, 'dep', 'proc'
            )**2,
            4, 'proc', 'dep'
        )
        ...
        success = (
            self.ideal_success[nqubits] * circ_dep * prep_dep * meas_dep 
            + (1 - circ_dep * meas_dep * prep_dep)/2
        )
